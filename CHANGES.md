...
- **Module bundling**: Bundle multi-file Python projects into a single JavaScript file
  - `py2js entry.py -o out.js --bundle -M src/` bundles entry file and all imports
  - Recursively resolves and bundles all imported modules
  - Topologically sorts modules by dependencies (dependencies first)
  - Combined tree-shaking: stdlib includes only functions used by ANY bundled module

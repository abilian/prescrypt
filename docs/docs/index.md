...
The prescrypt module provides functionality for transpiling Python code
to JavaScript.
...
`py2js <prescrypt.py2js>`{.interpreted-text role="func"} and
`evalpy <prescrypt.evalpy>`{.interpreted-text role="func"}. In principal
you do not need knowledge of JavaScript to write PScript code, though it
does help in corner cases.

## Goals

There is an increase in Python projects that target web technology to
handle visualization and user interaction. PScript grew out of a desire
to allow writing JavaScript callbacks in Python, to allow user-defined
interaction to be flexible, fast, and stand-alone.
...
Python snippet can be converted to JS; you don\'t need another JS
library to run it.
...
This means that depending on what you want to achieve, you may still
need to know a thing or two about how JavaScript works. Further, not all
Python code can be converted (e.g. import is not supported), and lists
and dicts are really just JavaScript arrays and objects, respectively.

## Pythonic

PScript makes writing JS more \"Pythonic\". Apart from allowing Python
syntax for loops, classes, etc, all relevant Python builtins are
supported, as well as the methods of list, dict and str. E.g. you can
use `print()`, `range()`, `L.append()`, `D.update()`, etc.

The empty list and dict evaluate to false (whereas in JS it\'s true),
and `isinstance()` just works (whereas JS\' `typeof` is broken).

Deep comparisons are supported (e.g. for `==` and `in`), so you can
compare two lists or dicts, or even a structure of nested lists/dicts.
Lists can be combined with the plus operator, and lists and strings can
be repeated with the multiply (star) operator. Class methods are bound
functions.

## Caveats {#pscript-caveats}
...
-   Magic functions on classes (e.g. for operator overloading) do not
    work.
...
-   A function can accept keyword arguments if it has a `**kwargs`
    parameter or named arguments after `*args`. Passing keywords to a
    function that does not handle keyword arguments might result in
    confusing errors.
-   Divide by zero results in [inf]{.title-ref} instead of raising
    ZeroDivisionError.
-   In Python you can do [a_list += a_string]{.title-ref} where each
    character in the string will be added to the list. In PScript this
    will convert [a_list]{.title-ref} to a string.

## PScript is valid Python

Other than e.g. RapydScript, PScript is valid Python. This allows
creating modules that are a mix of real Python and PScript. You can
easily write code that runs correctly both as Python and PScript, and
`raw JavaScript <prescrypt.RawJS>`{.interpreted-text role="func"} can be
included where needed (e.g. for performance).

PScript\'s compiler is written in Python. Perhaps PScript can at some
point compile itself, so that it becomes possible to define PScript
...
Faster than CPython, and significantly faster than e.g. Brython. Check
out `examples/app/benchmark.py`.
...
performance critical code in pure JavaScript (using
`RawJS <prescrypt.RawJS>`{.interpreted-text role="func"}) if necessary.

## Using PSCRIPT_OVERLOAD to increase performance {#pscript-overload}
...
follows will not be subject to overloading. This parser setting can only
be used inside a function and applies only to (the scope of) that
...
-   The add operator (`+`), so list concatenation cannot be done with
    `+`.
-   The multiply operator (`*`), so repeating a list or string cannot be
    done with `*`.
-   The equals operator (`==`), so deep comparisons of tuples/lists and
    dicts does not work.
-   The implicit truthy operator (as e.g. used in an if-statement), so
    empty tuples/lists and dicts evaluate to True. Note that functions
    like `bool()`, `all()` and `any()` still use the overloaded truthy.

## Support {#pscript-support}
...
-   numbers, strings, lists, dicts (the latter become JS arrays and
    objects)
-   operations: binary, unary, boolean, power, integer division, `in`
    operator
-   comparisons (`==` -\> `==`, `is` -\> `===`)
...
-   while-loops and for-loops supporting continue, break, and
    else-clauses
-   for-loops using `range()`
-   for-loop over arrays
-   for-loop over dict/object using `.keys()`, `.values()` and
    `.items()`
...
-   all Python builtin functions that make sense in JS are supported:
    ...
    delattr, print, len, max, min, chr, ord, dict, list, tuple, range,
    pow, sum, round, int, float, str, bool, abs, divmod, all, any,
...
-   the default return value of a function is `None`/`null` instead of
    `undefined`.
...
-   class methods are bound functions (i.e. `this` is fixed to the
    instance).
-   functions that are defined in another function (a.k.a closures) that
    do not have self/this as a first argument, are bound the the same
    instance as the function in which it is defined.

## Other functionality

The PScript package provides a few other \"utilities\" to deal with JS
code, such as renaming function/class definitions, and creating JS
modules (AMD, UMD, etc.).

## Credits

Prescrypt is a fork of [PScript]() with additional inspirations.

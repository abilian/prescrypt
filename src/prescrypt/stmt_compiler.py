...
class StatementCompiler(ExpressionCompiler):
    ...
    def gen_raise(self, exc, cause):
        ...
        match exc:
            case ast.Name():
                if exc.name[0].islower():  # raise an (error) object
                    return [self.lf("throw " + err_node.name + ";")]
                err_cls = err_node.name
            case ast.Call(func, args, keywords):
                err_cls = func.name
                err_msg = "".join([unify(self.gen_expr(arg)) for arg in args])
            case _:
                err_msg = "".join(self.gen_expr(exc))
    ...
    def gen_assert(self, test_node, msg_node):
        js_test = self.gen_expr(test_node)
        js_msg = js_test
        if msg_node:
            js_msg = self.gen_expr(msg_node)
    ...
    def gen_try(self, body, handlers, orelse, finalbody):
        if orelse:
    ...
    def gen_excepthandler(self, node):
        err_name = "err_%i" % self._indent

...
from prescrypt import RawJS, evaljs, evalpy, py2js
...
class TestHardcoreBuiltins:
    def test_isinstance(self):
        ...
        assert evalpy("isinstance(3.0, list) is True") is False
        assert evalpy("isinstance(3.0, float) is True") is True

        assert evalpy('x={}; isinstance(x.foo, "undefined")') is True

        assert evalpy('isinstance(None, "null")') is True
        assert evalpy('isinstance(undefined, "undefined")') is True
        #
        assert evalpy('isinstance(None, "undefined")') is False
        assert evalpy('isinstance(undefined, "null")') is False

        assert evalpy("isinstance(3, float)") is True
        assert evalpy("isinstance(3, (int, float))") is True
        assert evalpy('isinstance(3, "number")') is True
        #
        # assert evalpy('isinstance(3, int)') == 'false'  # int is not defined

        assert evalpy('isinstance("", str)') is True
        assert evalpy('isinstance("", "string")') is True
        #
        assert evalpy('isinstance("", list)') is False

        assert evalpy("isinstance(True, bool)") is True
        assert evalpy('isinstance(True, "boolean")') is True
        #
        assert evalpy("isinstance(True, float)") is False

        assert evalpy("isinstance([], list)") is True
        assert evalpy('isinstance([], "array")') is True
        #
        assert evalpy('isinstance([], "object")') is False
        assert evalpy('isinstance([], "Object")') is False
        assert evalpy("isinstance([], dict)") is False

        assert evalpy("isinstance({}, dict)") is True
        assert evalpy('isinstance({}, "object")') is True
        #
        assert evalpy("isinstance({}, list)") is False
        assert evalpy('isinstance({}, "array")') is False

        assert evalpy("isinstance(eval, types.FunctionType)") is True
        assert evalpy("isinstance(eval, FunctionType)") is True
        assert evalpy("isinstance(3, types.FunctionType)") is False

        # own class
        code = "function MyClass () {return this;}\nvar x = new MyClass();\n"
        assert evaljs(code + py2js('isinstance(x, "object")')) is True
        assert evaljs(code + py2js('isinstance(x, "Object")')) is True
        assert evaljs(code + py2js('isinstance(x, "MyClass")')) is True
        assert evaljs(code + py2js("isinstance(x, MyClass)")) is True

    def test_issubclass(self):
        code = "class Foo:pass\nclass Bar(Foo): pass\n"
        assert evalpy(code + "issubclass(Bar, Foo)") is True
        assert evalpy(code + "issubclass(Foo, Bar)") is False
        assert evalpy(code + "issubclass(Bar, object)") is True

    def test_hasattr(self):
        code = 'a = {"foo":1, "bar":2};\n'
        assert evalpy(code + 'hasattr(a, "foo")') is True
        assert evalpy(code + 'hasattr(a, "fooo")') is False
    ...
    def test_callable(self):
        assert evalpy("callable([])") is False
        assert evalpy("callable(3)") is False

        assert evalpy("callable(Boolean)") is True
        assert evalpy("callable(eval)") is True
        assert evalpy("def foo():pass\ncallable(foo)") is True
    ...
    def test_dict(self):
        ...
        assert evalpy('dict([["foo", 1], ["bar", 2]])') == {"foo": 1, "bar": 2}
        assert evalpy('dict({"foo": 1, "bar": 2})') == {"foo": 1, "bar": 2}
...
class TestOtherBuiltins:
    ...
    def test_bool(self):
        assert evalpy("bool(5)") is True
        assert evalpy('bool("xx")') is True
        assert evalpy("bool(0)") is False
        assert evalpy('bool("")') is False
        #
        assert evalpy("bool([1])") is True
        assert evalpy("bool({1:2})") is True
        assert evalpy("bool([])") is False
        assert evalpy("bool({})") is False
    ...
    def test_all(self):
        assert evalpy("all([1, 2, 3])") is True
        assert evalpy("all([0, 2, 3])") is False
        assert evalpy("all([])") is True

        assert evalpy("all([3, [], 3])") is False
        assert evalpy("all([3, [1], 3])") is True

    def test_any(self):
        assert evalpy("any([1, 2, 3])") is True
        assert evalpy("any([0, 2, 0])") is True
        assert evalpy("any([0, 0, 0])") is False
        assert evalpy("any([])") is False

        assert evalpy("any([0, [], 0])") is False
        assert evalpy("any([0, [1], 0])") is True

    def test_enumerate(self):
        assert evalpy("list(enumerate([10, 20, 30]))") == [[0, 10], [1, 20], [2, 30]]
    ...
    def test_reversed(self):
        assert evalpy("list(reversed([10, 20, 30]))") == [30, 20, 10]
    ...
    def test_filter(self):
        code = "list(filter(lambda x: x > 0, [-1, -2, 1, 2]))"
    ...
    def test_map(self):
        ...
        assert evalpy(code) == [1, 2, 4]


class TestListMethods:
    def test_append(self):
        assert evalpy("a = [2]; a.append(3); a") == [2, 3]
    ...
    def test_remove(self):
        ...
        # )
        assert evalpy('x = {"a":[2, 3]}; x.a.remove(2); x.a') == [3]
    ...
    def test_sort(self):
        ...
        assert evalpy("a=[3,1,4,2]; a.sort(key=lambda x: -x, reverse=True); a") == [
            1,
            2,
            3,
            4,
        ...
        assert evalpy('a=["bb", "aa", "mm", "dd"]; a.sort(); a') == [
            "aa",
            "bb",
            "dd",
            "mm",
        ]
        assert evalpy('a=["bb", "aa", "mm", "dd"]; a.sort(key=lambda x: x); a') == [
            "aa",
            "bb",
            "dd",
            "mm",
    ...
    def test_that_all_list_methods_are_tested(self):
        tested = {x.split("_")[1] for x in dir(self) if x.startswith("test_")}
        needed = {x for x in dir(list) if not x.startswith("_")}
        ignore = ""
...
class TestDictMethods:
    def test_get(self):
        ...
        assert evalpy('a = {"foo":3}; a.get("bar")') == None
        assert evalpy('a = {"foo":3}; a.get("bar", 0)') == 0
        assert evalpy('{"foo":3}.get("foo")') == 3
        assert evalpy('{"foo":3}.get("bar", 0)') == 0

    def test_items(self):
        assert evalpy("d={'a':1, 'b':2, 3:3}; d.items()") == [
            ["3", 3],
            ["a", 1],
            ["b", 2],
        ]
        assert evalpy("d={}; d.items()") == []

    def test_keys(self):
        assert evalpy("d={'a':1, 'b':2, 3:3}; d.keys()") == ["3", "a", "b"]
        assert evalpy("d={}; d.keys()") == []
    ...
    def test_values(self):
        assert evalpy("d={'a':1, 'b':2, 3:3};d.values()") == [3, 1, 2]
        assert evalpy("d={};d.values()") == []
    ...
    def test_that_all_dict_methods_are_tested(self):
        tested = {x.split("_")[1] for x in dir(self) if x.startswith("test_")}
        needed = {x for x in dir(dict) if not x.startswith("_")}
        ignore = "fromkeys"
        needed = needed.difference(ignore.split(" "))
...
class TestStrMethods:
    ...
    def test_endswith(self):
        assert evalpy('"blafoo".endswith("foo")') is True
        assert evalpy('"blafoo".endswith("")') is True
        assert evalpy('"foo".endswith("foo")') is True
        assert evalpy('"".endswith("foo")') is False
        assert evalpy('"".endswith("")') is True
        assert evalpy('"a".endswith("bb")') is False  # See issue #66

    def test_startswith(self):
        assert evalpy('"foobla".startswith("foo")') is True
        assert evalpy('"foobla".startswith("")') is True
        assert evalpy('"foo".startswith("foo")') is True
        assert evalpy('"".startswith("foo")') is False
        assert evalpy('"".startswith("")') is True

        assert evalpy('("fo" + "obar").startswith("foo")') is True
    ...
    def test_isalnum(self):
        assert evalpy('"".isalnum()') is False
        assert evalpy('"012".isalnum()') is True
        assert evalpy('"abc".isalnum()') is True
        assert evalpy('"0a1b2c".isalnum()') is True
        assert evalpy('"0a_".isalnum()') is False

    def test_isalpha(self):
        assert evalpy('"".isalpha()') is False
        assert evalpy('"012".isalpha()') is False
        assert evalpy('"abc".isalpha()') is True
        assert evalpy('"0a1b2c".isalpha()') is False
        assert evalpy('"0a_".isalpha()') is False

    def test_isdecimal(self):
        assert evalpy('"".isdecimal()') is False
        assert evalpy('"012".isdecimal()') is True
        assert evalpy('"abc".isdecimal()') is False
        assert evalpy('"0a1b2c".isdecimal()') is False
        assert evalpy('"0a_".isdecimal()') is False

    def test_isdigit(self):
        assert evalpy('"".isdigit()') is False
        assert evalpy('"012".isdigit()') is True
        assert evalpy('"abc".isdigit()') is False
        assert evalpy('"0a1b2c".isdigit()') is False
        assert evalpy('"0a_".isdigit()') is False

    def test_isnumeric(self):
        assert evalpy('"".isnumeric()') is False
        assert evalpy('"012".isnumeric()') is True
        assert evalpy('"abc".isnumeric()') is False
        assert evalpy('"0a1b2c".isnumeric()') is False
        assert evalpy('"0a_".isnumeric()') is False

    def test_isidentifier(self):
        assert evalpy('"".isidentifier()') is False
        assert evalpy('"012".isidentifier()') is False
        assert evalpy('"abc".isidentifier()') is True
        assert evalpy('"0a1b2c".isidentifier()') is False
        assert evalpy('"a0a1b2c".isidentifier()') is True
        assert evalpy('"0a_".isidentifier()') is False
        assert evalpy('"_a".isidentifier()') is True
        assert evalpy('"_0".isidentifier()') is True

    def test_islower(self):
        assert evalpy('"".islower()') is False
        assert evalpy('" ".islower()') is False
        assert evalpy('"aBc".islower()') is False
        assert evalpy('"aBc 01_".islower()') is False

        assert evalpy('"abc".islower()') is True
        assert evalpy('"abc 01_".islower()') is True

    def test_isupper(self):
        assert evalpy('"".isupper()') is False
        assert evalpy('" ".isupper()') is False
        assert evalpy('"AbC".isupper()') is False
        assert evalpy('"AbC 01_".isupper()') is False

        assert evalpy('"ABC".isupper()') is True
        assert evalpy('"ABC 01_".isupper()') is True

    def test_isspace(self):
        assert evalpy('"".isspace()') is False
        assert evalpy('" ".isspace()') is True
        assert evalpy('" \\t\\n".isspace()') is True
        assert evalpy('" _".isspace()') is False
        assert evalpy('" a".isspace()') is False
        assert evalpy('" 1".isspace()') is False

    def test_istitle(self):
        assert evalpy('"".istitle()') is False
        assert evalpy('" ".istitle()') is False
        assert evalpy('"AbC".istitle()') is False
        assert evalpy('"Foo bar".istitle()') is False
        assert evalpy('"AbC 01_".istitle()') is False

        assert evalpy('"Foo".istitle()') is True
        assert evalpy('"Foo Bar".istitle()') is True
        assert evalpy('"Foo 01_".istitle()') is True
    ...
    def test_that_all_str_methods_are_tested(self):
        tested = {x.split("_")[1] for x in dir(self) if x.startswith("test_")}
        needed = {x for x in dir(str) if not x.startswith("_")}

...
import prescrypt
from prescrypt import Parser, evaljs, evalpy, py2js
from prescrypt.parser0 import JSError
...
class TestExpressions:
    ...
    def test_ops(self):
        ...
        assert evalpy("not True") is False  # Unary
        assert evalpy("0-3") == -3
        assert evalpy("True and False") is False  # Boolean
        assert evalpy("True or False") is True
        # Bug
    ...
    def test_comparisons(self):
        ...
        assert evalpy("4 > 4") is False
        assert evalpy("4 >= 4") is True
        assert evalpy("4 < 3") is False
        assert evalpy("4 <= 4") is True
        assert evalpy("4 == 3") is False
        assert evalpy("4 != 3") is True

        assert evalpy('4 == "4"') is True  # yuck!
        assert evalpy('4 is "4"') is False
        assert evalpy('4 is not "4"') is True

        assert evalpy('"c" in "abcd"') is True
        assert evalpy('"x" in "abcd"') is False
        assert evalpy('"x" not in "abcd"') is True

        assert evalpy("3 in [1,2,3,4]") is True
        assert evalpy("9 in [1,2,3,4]") is False
        assert evalpy("9 not in [1,2,3,4]") is True

        assert evalpy('"bar" in {"foo": 3}') is False
        assert evalpy('"foo" in {"foo": 3}') is True

        # was a bug
        assert evalpy("not (1 is null and 1 is null)") is True

    def test_deep_comparisons(self):
        # List
        arr = "[(1,2), (3,4), (5,6), (1,2), (7,8)]\n"
        assert evalpy("a=" + arr + "(1,2) in a") is True
        assert evalpy("a=" + arr + "(7,8) in a") is True
        assert evalpy("a=" + arr + "(3,5) in a") is False
        assert evalpy("a=" + arr + "3 in a") is False

        assert evalpy("(2, 3) == (2, 3)") is True
        assert evalpy("[2, 3] == [2, 3]") is True
        assert evalpy("a=" + arr + "b=" + arr + "a==b") is True

        # Dict
        dct = '{"a":7, 3:"foo", "bar": 1, "9": 3}\n'
        assert evalpy("d=" + dct + '"a" in d') is True
        assert evalpy("d=" + dct + '"3" in d') is True
        assert evalpy("d=" + dct + "3 in d") is True
        assert evalpy("d=" + dct + '"bar" in d') is True
        assert evalpy("d=" + dct + "9 in d") is True
        assert evalpy("d=" + dct + '"9" in d') is True
        assert evalpy("d=" + dct + "7 in d") is False
        assert evalpy("d=" + dct + '"1" in d') is False

        assert evalpy('{2: 3} == {"2": 3}') is True
        assert evalpy('dict(foo=7) == {"foo": 7}') is True
        assert evalpy("a=" + dct + "b=" + dct + "a==b") is True
        assert evalpy('{"foo": 1, "bar": 2}=={"bar": 2, "foo": 1}') is True
        assert evalpy('{"bar": 2, "foo": 1}=={"foo": 1, "bar": 2}') is True

        # Deeper
        d1 = 'd1={"foo": [2, 3, {1:2,3:4,5:["aa", "bb"]}], "bar": None}\n'
        d2 = 'd2={"bar": None, "foo": [2, 3, {5:["aa", "bb"],1:2,3:4}]}\n'  # same
        d3 = 'd3={"foo": [2, 3, {1:2,3:4,5:["aa", "b"]}], "bar": None}\n'  # minus b
        assert evalpy(d1 + d2 + d3 + "d1 == d2") is True
        assert evalpy(d1 + d2 + d3 + "d2 == d1") is True
        assert evalpy(d1 + d2 + d3 + "d1 != d2") is False
        assert evalpy(d1 + d2 + d3 + "d1 == d3") is False
        assert evalpy(d1 + d2 + d3 + "d1 != d3") is True
        #
        assert evalpy(d1 + d2 + d3 + "d2 in [2, d1, 4]") is True
        assert evalpy(d1 + d2 + d3 + 'd2 in ("xx", d2, None)') is True
        assert evalpy(d1 + d2 + d3 + "d2 not in (1, d3, 2)") is True
        assert evalpy(d1 + d2 + d3 + "4 in [2, d1, 4]") is True
    ...
    def test_truthfulness_of_basic_types(self):
        ...
        # None - undefined
        assert evalpy("None is null") is True
        assert evalpy("None is undefined") is False
        assert evalpy("undefined is undefined") is True
    ...
    def test_ignore_import_of_compiler(self):
        modname = prescrypt.__name__
        assert py2js(f"from {modname} import x, y, z\n42") == "42;"

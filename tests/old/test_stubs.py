...
import prescrypt
from prescrypt import RawJS


def test_stubs():
    from prescrypt.stubs import omgnotaname, undefined, window

    assert isinstance(window, prescrypt.JSConstant)
    assert isinstance(undefined, prescrypt.JSConstant)
    assert isinstance(omgnotaname, prescrypt.JSConstant)

...
from prescrypt import JSError, evaljs, evalpy, py2js
...
class TestConrolFlow:
    ...
    def test_for(self):
        ...
        assert evaljs(f"{code} method_for()") == "ok\nok\nnull"
    ...
    def test_list_comprehensions(self):
        ...
        assert eval(code1) == evalpy(code1)
        #
        code2 = "a = " + code1 + "; a == " + code1
        assert evalpy(code2) is True
...
class TestExceptions:
    def test_raise(self):
        ...
        # FIXME: not supported by duktape
    ...
    def test_finally(self):
        ...
        assert evaljs(py2js(catchtest, "f") + "f(1)") == "xxyy"
...
class TestContextManagers:
    def test_with_simple(self):
        def contexttest():
            ...
            c = dict(__enter__=enter, __exit__=exit)
            with c:
                result.append(42)
            result.append(".")
            return result

        assert evaljs(py2js(contexttest, "f") + "f()") == ["enter", 42, "exit", "."]
    ...
    def test_with_as1(self):
        ...
        assert evaljs(py2js(contexttest, "f") + "f()") == [
            "enter",
            42,
            7,
            43,
    ...
    def test_with_as2(self):
        ...
        assert evaljs(py2js(contexttest, "f") + "f()") == contexttest()
...
class TestFunctions:
    ...
    def test_var_args1(self):
        ...
        assert evaljs(code1 + code2) == [2, 3]
        code2 = py2js("func()")
        assert evaljs(code1 + code2) == []
        code2 = py2js("a=[2,3]\nfunc(*a)")
        assert evaljs(code1 + code2) == [2, 3]
        code2 = py2js("a=[2,3]\nfunc(1,2,*a)")
        assert evaljs(code1 + code2) == [1, 2, 2, 3]
    ...
    def test_global_vs_nonlocal(self):
        ...
        assert js1.meta["vars_global"] == {"foo"}
...
class TestClasses:
    ...
    def test_class_name_mangling1(self):
        ...
        assert evaljs(code + "m.__x;") == None
        assert evaljs(code + "m.__y;") == None
    ...
    def test_inheritance_and_super(self):
        ...
        class MyClass3(MyClass2):
            ...
            def addFour(self):
                super().add(4)  # Use legacy Python syntax
        ...
        # Inhertance m1
        assert evaljs(code + "m1 instanceof MyClass3;") is False
        assert evaljs(code + "m1 instanceof MyClass2;") is False
        assert evaljs(code + "m1 instanceof MyClass1;") is True
        assert evaljs(code + "m1 instanceof Object;") is True

        # Inhertance m2
        assert evaljs(code + "m2 instanceof MyClass3;") is False
        assert evaljs(code + "m2 instanceof MyClass2;") is True
        assert evaljs(code + "m2 instanceof MyClass1;") is True
        assert evaljs(code + "m2 instanceof Object;") is True

        # Inhertance m3
        assert evaljs(code + "m3 instanceof MyClass3;") is True
        assert evaljs(code + "m3 instanceof MyClass2;") is True
        assert evaljs(code + "m3 instanceof MyClass1;") is True
        assert evaljs(code + "m3 instanceof Object;") is True
    ...
    def test_inheritance_super_more(self):
        ...
        assert evaljs(code + "m4.foo() === m4") is True
        assert evaljs(code + "m4.foo() === m4") is True
        assert evaljs(code + "foo.call(m4) === m4") is True
    ...
    def test_bound_methods(self):
        ...
        assert (
            evaljs(code + "var m = new MyClass14(); var f = m.add2; f(); f(); m.a") == 5
        )
        assert (
            evaljs(code + "var m = new MyClass15(); var f = m.add3; f(); f(); m.a") == 7
        )
        assert (
            evaljs(
                code
